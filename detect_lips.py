...
sys.path.insert(1, os.path.join(os.path.dirname(__file__), 'landmark_detection/hrnet_lms/experiments/300w/face_alignment_300w_hrnet_w18.yaml'))
...
class LipDetector():
  ...
  def load_image(self, image_path) -> np.ndarray:
    ...
    img = img / 255. * 2 - 1 # Map values to -1.0 to 1.0
  ...
  def detect_lips(self, preprocessed_img):
    ...
    return [marks[[self.LEFT_LIP_CORNER, self.RIGHT_LIP_CORNER]].copy()], heatmap_grid
