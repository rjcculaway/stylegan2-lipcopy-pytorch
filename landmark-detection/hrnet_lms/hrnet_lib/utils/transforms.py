...
def crop(img, center, scale, output_size, rot=0):
    ...
    if sf < 2:
        ...
        if new_size < 2:
            ...
            img = np.array(Image.fromarray(img.astype(np.uint8)).resize([new_ht, new_wd])).astype(float)
    ...
    new_img = np.array(Image.fromarray(new_img.astype(np.uint8)).resize(output_size)).astype(float)
    return new_img
